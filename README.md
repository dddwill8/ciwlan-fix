...
只改运行时电话接口，不刷分区、不写 SIM。卡 2 锁无服务会写成 modem 里的手动选网（联通 / 电信 / 广电，关掉开关会还原）。**不需要 HyperCeiler。**
...
- 卡 2 不会去驻留国内漫游网（Ultra）。默认手动选中国电信；可在模块设置里改成广电或联通。驻上 460 会再踢回所选网。
- 状态栏可以出现 VoWiFi 图标。
...
打开开关就行。卡 2 默认锁在**中国电信**；模块设置里可以改成广电或联通。电信/广电一般注册不上，联通有可能被 T-Mobile 漫游上，优先别选。
...
完整日志在电脑看，或在模块页点「复制 adb 日志命令」「导出日志到 Download」。
