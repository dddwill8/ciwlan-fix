...
当前 **test/radio-off-slot2** 测试分支：备用通话起来后关掉卡 2 蜂窝无线电（可还原）。正式版仍在 `main` / [v1.0.4](https://github.com/dddwill8/ciwlan-fix/releases/tag/v1.0.4)。
...
- 卡 2 不会去驻留国内网。测试分支在 IWLAN 起来后关掉卡 2 射频；若备用通话因此掉线会自动开回射频并停止重试。关开关会开回射频。
- 状态栏可以出现 VoWiFi 图标。
...
1. 先关掉模块里的开关，等卡 2 射频开回来（看设置页或日志 `setRadioPower(true)`）。
2. 再去 LSPosed 禁用模块，重启。
3. 最后才卸 APK。

直接禁模块或直接卸，卡 2 可能还停在关射频。必须先关开关还原。要回到正式版，装回 v1.0.4。
