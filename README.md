...
只改运行时电话接口，不刷分区、不写 SIM。

当前 **test/disable-wwan-slot2** 测试分支：卡 2 禁 GSM/LTE/NR，只留 IWLAN/跨卡，不再用假 PLMN `99999`。正式版仍在 `main` / [v1.0.4](https://github.com/dddwill8/ciwlan-fix/releases/tag/v1.0.4)。
...
- 卡 2 不会去驻留国内网，避免 Ultra 这类漫游。测试分支通过关掉卡 2 的 WWAN 制式，让基带不去扫 460。
- 状态栏可以出现 VoWiFi 图标。
...
1. 先关掉模块里的开关，等它把卡 2 的网络制式改回去（看设置页「卡 2 USER 制式 bitmask」不再是 0）。
2. 再去 LSPosed 禁用模块，重启。
3. 最后才卸 APK。

直接禁模块或直接卸，卡 2 可能还停在禁 WWAN / 无服务。必须先关开关还原。要回到正式版，装回 `main` 的 v1.0.4 即可。
