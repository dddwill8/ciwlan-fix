...
当前 **test/gsm-only-and-ux** 测试分支：卡 2 只留 2G、IMS 掉线会重连、挡住选网失败通知，并把 SIM 信息页的 WLAN 通话 / 通话辅助勾选和底层对齐。正式版仍在 `main` / [v1.0.4](https://github.com/dddwill8/ciwlan-fix/releases/tag/v1.0.4)。
...
- 卡 2 不会去驻留国内 LTE/NR（测试分支锁 GSM-only）。IMS 掉线会重新注入 IWLAN。选网失败通知会被挡掉。
- 状态栏可以出现 VoWiFi 图标。
...
1. 先关掉模块里的开关，等卡 2 网络制式改回 LTE/NR（设置页 bitmask 不再是 GSM-only）。
2. 再去 LSPosed 禁用模块，重启。
3. 最后才卸 APK。

直接禁或直接卸，卡 2 可能还停在 2G-only。要回正式版，装 v1.0.4。
