...
/** Drop slot-2 PLMN / no-service toasts so the 25-minute scan is less visible. */
final class NotifyGuard {
    ...
    private static boolean block(Object[] args) {
        ...
        if (s.contains("99999")
                ...
                || (s.contains("无服务") && (s.contains("t-mobile") || s.contains("卡2")
                || s.contains("卡 2") || s.contains("sim 2") || s.contains("esim")))) {
